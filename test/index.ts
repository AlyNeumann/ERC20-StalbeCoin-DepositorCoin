...
describe("StableCoin", function () {
  ...
  it("Should set the fee rate percentage", async function () {
    expect(await StableCoin.feeRatePercentage()).to.equal(feeRatePercentage);
  });

  it("Shoud allow minting", async function() {
    const ethAmount = 1;
  ...
  describe("With minted tokens", function () {
    ...
    it("Should prevent depositing collateral buffer below minimum", async function () {
      ...
      await expect(
        ...
        `custom error 'InitialCollateralRatioError("Initial collateral ratio not met, minimum is: ", ${ethers.utils.parseEther(expectedMinimumAmount.toString())})'`
      )
    })
    ...
    describe("With depostied collateral buffer", function () {
      ...
      it("Should allow withdrawing collateral buffer", async function () {
        const newDepositorTotalySupply = stableCoinCollateralBuffer * ethUsdPrice;
        const stableCoinCollateralBurnAmount = newDepositorTotalySupply * 0.2;

        await StableCoin.withrawCollateralBuffer(
          ethers.utils.parseEther(stableCoinCollateralBurnAmount.toString())
        );

        expect(await DepositorCoin.totalSupply()).to.equal(
          ethers.utils.parseEther(
            (newDepositorTotalySupply - stableCoinCollateralBurnAmount).toString()
          )
        );
      });
    })
